...
while True:
    ...
    _, otsu = cv2.threshold(
        blur, 125, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    _, contours, hierarchy = cv2.findContours(
        otsu.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)
    max_area = -1
    l = len(contours)
    for i in range(l):
        cnt = contours[i]
    ...
    if count_defects == 1:
        cv2.putText(frame, '2 ', (0, 130), font,
        ...
        cv2.putText(frame, 'gime me command', (0, 130),
